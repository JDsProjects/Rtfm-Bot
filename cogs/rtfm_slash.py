...
class LibraryTransformer(Transformer):
    ...
    async def autocomplete(
        ...
        choices = [
            Choice(name=name, value=name) for name in interaction.client.rtfm_libraries
        ]
        start_with = (
            list(filter(lambda x: x.name.startswith(current), choices)) or choices
        )
...
class QueryTransformer(Transformer):
    async def transform(
        ...
        library = interaction.client.rtfm_libraries.get(
            interaction.namespace.library, interaction.namespace.library
        )
...
class DocsQueryTransformer(Transformer):
    ...
    async def autocomplete(
        ...
        all_choices = [
            Choice(name=result.name, value=result.name) for result in unfiltered_results
        ]
        ...
        results = [
            Choice(name=result.name, value=result.name) for result in filtered_results
        ]
...
class RTFMSlash(commands.Cog):
    ...
    @rtfm_slash.error
    async def rtfm_error(self, interaction: Interaction[RTFMBot], error) -> None:
